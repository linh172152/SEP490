...
export default function RoomDetailPage() {
  ...
  const loadRoomData = useCallback(async () => {
    try {
      ...
      setRoom(roomData);
      setElderlyInRoom(allElderly.filter((e: ElderlyProfileResponse) => e.roomId === roomId));
      setCaregiversInRoom(allCaregivers.filter((c: AccountResponse) => c.roomId === roomId));
      setAvailableElderly(allElderly.filter((e: ElderlyProfileResponse) => !e.roomId));
      setAvailableCaregivers(allCaregivers.filter((c: AccountResponse) => !c.roomId));
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">{roomLabel}</h1>
      </div>
      ...
      <Tabs defaultValue="elderly" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="elderly">Elderly ({elderlyInRoom.length})</TabsTrigger>
          <TabsTrigger value="caregivers">Caregivers ({caregiversInRoom.length})</TabsTrigger>
        </TabsList>
                ...
                <TableBody>
                  {elderlyInRoom.map((elderly) => (
                    <TableRow key={elderly.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{elderly.healthNotes || 'Good'}</Badge>
                      </TableCell>
                      <TableCell>
                        <Button
                          variant="outline"
