...
export default function CaregiverCareTasksPage() {
  ...
  const handleOpenEditReminder = (reminder: ReminderResponse) => {
    ...
    setReminderForm({
      ...
      caregiverId: profile?.accountId || (user?.id ? Number(user.id) : 0) || profile?.id || reminder.caregiverId,
  ...
  const handleSubmitReminder = async () => {
    const effectiveCaregiverId = profile?.accountId || (user?.id ? Number(user.id) : undefined) || profile?.id;
