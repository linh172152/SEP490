...
import { ArrowRight, Bot, HeartPulse, ShieldCheck, Cpu } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <motion.div variants={staggerItem} className="flex justify-center lg:justify-start mb-6">
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-bold tracking-widest uppercase">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-primary"></span>
                </span>
                {t('landing.hero.badge', 'Modern Care Ecosystem')}
              </div>
            </motion.div>

            <motion.h1 
              variants={staggerItem}
              className="text-4xl font-black tracking-tight sm:text-6xl lg:text-7xl text-foreground"
            >
              {t('landing.hero.title_main', 'The Intelligent')}{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-primary/80 to-primary/50">
                {t('landing.hero.title_highlight', 'Eldercare Ecosystem')}
              </span>
            </motion.h1>
            ...
            >
              {t('landing.hero.subtitle', 'CareBot-MH empowers caregivers, managers, and families with AI-driven insights and continuous compassionate robotic assistance for senior well-being.')}
            </motion.p>
            ...
            <motion.div variants={staggerItem} className="mt-10 flex items-center justify-center lg:justify-start gap-x-6">
              <Link href="/login" className="relative group">
                ...
                <Button size="lg" className="relative h-14 px-8 text-lg rounded-full shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95">
                  {t('landing.hero.cta_access', 'Access Portal')} <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </Link>
            </motion.div>
            ...
            <motion.div variants={staggerItem} className="mt-14 grid grid-cols-2 gap-y-8 sm:grid-cols-3 mx-auto lg:mx-0 max-w-lg lg:max-w-none items-start">
              <div className="flex flex-col items-center lg:items-start gap-2 text-muted-foreground group">
                ...
                <span className="text-sm font-medium text-center lg:text-left transition-colors group-hover:text-foreground">
                  {t('landing.hero.features.ai', 'Predictive AI Monitoring')}
                </span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-2 text-muted-foreground group">
                ...
                <span className="text-sm font-medium text-center lg:text-left transition-colors group-hover:text-foreground">
                  {t('landing.hero.features.sensing', 'Real-time Emotion Sensing')}
                </span>
              </div>
              <div className="col-span-2 sm:col-span-1 flex flex-col items-center lg:items-start gap-2 text-muted-foreground group">
                ...
                <span className="text-sm font-medium text-center lg:text-left transition-colors group-hover:text-foreground">
                  {t('landing.hero.features.security', 'Enterprise-Grade Security')}
                </span>
              </div>
            </motion.div>
          </motion.div>
