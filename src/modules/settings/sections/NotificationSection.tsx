    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-4 gap-4 border-b last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-900/50 rounded-lg -mx-2 px-2 transition-colors">
      ...
      <div className="flex items-center gap-6 sm:gap-8">
        <div className="flex flex-col items-center gap-2">
          ...
          <Switch 
            checked={prefs.email} 
            onCheckedChange={() => onToggle(categoryKey, 'email', prefs.email)}
            disabled={isSaving}
          />
        </div>
        <div className="flex flex-col items-center gap-2">
          ...
          <Switch 
            checked={prefs.sms} 
            onCheckedChange={() => onToggle(categoryKey, 'sms', prefs.sms)}
            disabled={isSaving}
          />
        </div>
        <div className="flex flex-col items-center gap-2">
          ...
          <Switch 
            checked={prefs.push} 
            onCheckedChange={() => onToggle(categoryKey, 'push', prefs.push)}
            disabled={isSaving}
          />
        </div>
      </div>
